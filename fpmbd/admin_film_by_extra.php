        ...
        <div class="row">
            <div class="col-md-12">
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                    <?php
                    $films = mysqli_query ($link, "
                        SELECT f.nama_film, SUM(t.jumlah_tiket) AS jumlah_tiket_terjual, SUM(t.total_pembayaran) AS pendapatan_kotor, f.*, p.*
                        FROM transaksi t
                             JOIN memutar m ON t.id_memutar = m.id_memutar
                             JOIN film f ON m.id_film = f.id_film
                             join produser p on f.id_produser = p.id_produser
                        GROUP BY f.nama_film
                        ORDER BY jumlah_tiket_terjual DESC;"
                    );
                    while ($row = mysqli_fetch_assoc ($films)) { ?>
                        <tr>
                            ...
                            <td class="text-center" style="vertical-align: middle">
                                <p><?php echo $row['jumlah_tiket_terjual'] ?></p>
                            </td>

                            <td class="text-center" style="vertical-align: middle">
                                <p><?php echo $row['pendapatan_kotor'] ?></p>
                            </td>
